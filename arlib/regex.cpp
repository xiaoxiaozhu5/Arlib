...
static const char * const insn_names[] = {
	...
	"dfa_shortest",
	"dfa_longest",
	"dfa_longest_possessive",
	"capture_start",
	"capture_end",
	"capture_discard",
	"capture_discard_force",
	"capture_backref",
	"ensure_progress",
	"assert_boundary",
	"assert_notboundary",
	...
	"lookahead_positive",
	"lookahead_negative",
	"lookahead_positive_complete",
	"lookahead_negative_complete",
};

class regex::parser {
	static_assert(ARRAY_SIZE(insn_names) == regex::t_lookahead_negative_complete+1);
...
public:
	...
	struct node_t {
		...
		array<node_t> children; // valid for type=series, type=alternative, and subtype=lookahead
		                        // (lookahead doesn't use subtype.data, and has exactly 1 child node)
		
		// these five correspond to *+?{} repeats
		uint32_t num_total_bodies = 1;
		uint32_t num_mandatory_bodies = 1;
		bool has_repeat = false; // if false, the other four are at their default values
		bool repeat_infinite = false;
		bool greedy = true;
		
		bool can_nfa; // False if it contains any capture, assertion, or lookahead.
		bool can_accept_empty;
		uint32_t enforce_progress_idx = 0;
	};
...
private:
		...
		else if (*at == '(')
		{
			...
			if (at[1] == '?')
			{
				...
				else if (at[2] == '!')
				{
					...
					node_t& lookahead = target.children.append({ n_insn, { t_lookahead_negative }});
					at = parse_disjunction(lookahead.children.append(), at+3);
					if (!at || at == end || *at != ')')
						return parse_error();
					for (uint32_t i=n_prev_captures;i<rgx->num_captures;i++)
					{
						target.children.append({ n_insn, { t_capture_discard_force, i }});
	...
	static void flatten_singleton_sequences(node_t& target)
	{
		for (size_t i=0;i<target.children.size();i++)
		{
			node_t& ch = target.children[i];
			flatten_singleton_sequences(ch);
			if (ch.type == n_series && ch.children.size() <= 1 && !ch.has_repeat)
			{
				if (ch.children.size() == 1)
				{
					array<node_t> children = std::move(ch.children);
					ch = std::move(children[0]);
				}
				else if (target.type == n_series)
				{
...
public:
	...
	public:
		...
		static bool tag_can_nfa(node_t& node)
		{
			...
			if (node.type == n_insn)
			{
				can = (node.subtype.type == t_byte || is_dfa(node.subtype.type));
	...
	private:
		...
		void convert_single_node(regex* rgx, const node_t& node, uint32_t start_state, uint32_t accept_state)
		{
			if (node.type == n_insn)
			{
				...
				{
#ifndef ARLIB_OPT
					if (!is_dfa(node.subtype.type))
						abort();
#endif
					dfa_t& dfa = rgx->dfas[node.subtype.data];
					bool is_shortest = (node.subtype.type == t_dfa_shortest);
					
					array<uint32_t> states;
					states.resize(dfa.transitions.size());
					
					for (size_t i=0;i<dfa.transitions.size();i++)
						states[i] = new_state();
					add_immediate_transition(start_state, states[0], is_shortest);
					
					for (size_t i=0;i<dfa.transitions.size();i++)
					{
						map<uint32_t, bitset<256>> bytes;
						for (size_t j=0;j<256;j++)
							bytes.get_create(dfa.transitions[i].next[j]).set(j, true);
						for (auto& pair : bytes)
						{
							uint32_t next_st = (pair.key & 0x7FFFFFFF);
							if (next_st != 0x7FFFFFFF)
								add_transition(states[i], states[next_st], pair.value, is_shortest);
							if (pair.key & 0x80000000)
								add_transition(states[i], accept_state, pair.value, is_shortest);
						}
					}
					if (dfa.init_state & 0x80000000)
						add_immediate_transition(start_state, accept_state, is_shortest);
				}
			}
			else
			{
				create_nfa_inner(rgx, node.children, node.type==n_alternative, start_state, accept_state);
			}
		}
		...
		void create_nfa_inner(regex* rgx, arrayview<node_t> nodes, bool is_alternative, uint32_t start_state, uint32_t accept_state)
		{
			if (is_alternative)
			{
	...
	private:
		...
		// mostly exists to make debug output cleaner (reducing state count may make the nfa->dfa converter faster, not sure)
	...
	public:
		void create_nfa(regex* rgx, arrayview<node_t> nodes, bool is_alternative)
		{
			states.resize(2);
			states[1].accepting = true;
			// need to link the accept state to itself; otherwise, inlining an immediate to accept will return empty
			add_immediate_transition(1, 1, false);
			create_nfa_inner(rgx, nodes, is_alternative, 0, 1);
	...
	public:
		bool convert_to_dfa(dfa_t& dfa)
		{
			...
			while (!todo.empty() && dfa.transitions.size() < 256)
			{
				in_states = todo.pop();
				size_t n_set = 0;
		...
		// Ignores whether the inputs accept empty inputs, and misbehaves if they do. Caller must check for that.
		static bool convert_to_dfa_with_lookahead(nfa_t& physical, nfa_t& lookahead, bool lookahead_is_positive, dfa_t& dfa)
		{
			lookahead.set_unique_bytes();
			physical.set_unique_bytes();
			bitset<256> unique_bytes = lookahead.unique_bytes | physical.unique_bytes;
			...
			size_t n_physical_st = physical.states.size();
			size_t n_lookahead_st = lookahead.states.size();
			size_t n_lookaheadaccept_st = n_physical_st + n_lookahead_st;
			
			bitarray in_states;
			in_states.resize(n_physical_st + n_lookahead_st + 1);
			in_states[0] = true;
			in_states[n_physical_st] = true;
			todo.push(in_states);
			seen.get_create(in_states, 0);
			...
			while (!todo.empty() && dfa.transitions.size() < 256)
			{
				...
				for (size_t by : unique_bytes.true_idxs())
				{
					...
					bool sequel_in_lookahead = false;
					bool sequel_in_physical = false;
					bool lookahead_accepting = false;
					bool physical_accepting = false;
					...
					for (size_t i : in_states.true_idxs())
					{
						...
						{
							idx = i - 0;
							for (const transition_t& tr : physical.states[idx].transitions)
							{
								if (tr.bytes[by])
								{
									if (physical.states[tr.target].accepting)
										physical_accepting = true;
									if (physical.states[tr.target].accepts_sequel)
									{
										sequel_in_physical = true;
										new_states.set(tr.target, true);
									}
								}
							}
						}
						else if (i < n_physical_st+n_lookahead_st)
						{
							idx = i - n_physical_st;
							for (const transition_t& tr : lookahead.states[idx].transitions)
							{
								if (tr.bytes[by])
								{
									if (lookahead.states[tr.target].accepting)
										lookahead_accepting = true;
									if (lookahead.states[tr.target].accepts_sequel)
									{
										sequel_in_lookahead = true;
										new_states.set(tr.target + n_physical_st, true);
									}
								}
							}
						}
						...
						{
#ifndef ARLIB_OPT
							if (i != n_lookaheadaccept_st) abort();
#endif
							lookahead_accepting = true;
							sequel_in_lookahead = true;
						}
					}
					if (lookahead_is_positive)
					{
						if (physical_accepting && sequel_in_lookahead && !lookahead_accepting)
							return false;
						if (lookahead_accepting)
							new_states.set(n_lookaheadaccept_st, true);
						if (sequel_in_physical)
						{
							uint32_t create_state = seen.size();
							next_state = seen.get_create(new_states, create_state);
							if (next_state == create_state)
							...
							next_state = 0x7FFFFFFF;
						if (physical_accepting && lookahead_accepting)
							next_state |= 0x80000000;
					}
					else
					{
						if (physical_accepting && sequel_in_lookahead)
							return false;
						if (sequel_in_physical && !lookahead_accepting)
						{
							uint32_t create_state = seen.size();
							next_state = seen.get_create(new_states, create_state);
							if (next_state == create_state)
	...
	bool try_merge_lookaheads(node_t& node)
	{
		bool ret = false;
		for (size_t i=0;i<node.children.size();i++)
		{
			node_t& ch = node.children[i];
			if (node.type == n_series && ch.type == n_insn && is_lookahead(ch.subtype.type) && ch.children[0].can_nfa)
			{
#ifndef ARLIB_OPT
				if (ch.children.size() != 1) abort();
#endif
				bool is_positive = (ch.subtype.type == t_lookahead_positive);
				size_t end = i+1;
				while (end < node.children.size() && node.children[end].can_nfa)
					end++;
				nfa_t lookahead;
				lookahead.create_nfa(rgx, ch.children, false);
				if (lookahead.states[0].accepting)
				{
					// convert_to_dfa_with_lookahead doesn't like lookaheads that accept empty inputs
					// do not use things like can_accept_empty, they're true for things like $ which must not be removed
					// (though can_nfa is false for $)
					if (is_positive)
					{
					...
					{
						node.children[i] = { n_insn, { t_byte, (uint32_t)rgx->bytes.size() } };
						rgx->bytes.append();
					}
					ret = true;
					continue;
				...
				{
					nfa_t physical;
					physical.create_nfa(rgx, node.children.slice(i+1, end-(i+1)), false);
					
					if (!physical.states[0].accepting && physical.set_ordering())
					{
						uint32_t idx = rgx->dfas.size();
						bool ok = nfa_t::convert_to_dfa_with_lookahead(physical, lookahead, is_positive, rgx->dfas.append());
						if (ok)
						{
							node.children.remove_range(i+1, end);
							node.children[i] = { n_insn, { physical.shortest_first ? t_dfa_shortest : t_dfa_longest, idx } };
							ret = true;
							i--;
							continue;
						}
				...
				uint32_t idx = rgx->dfas.size();
				if (ch.children.size() > 1 && lookahead.convert_to_dfa(rgx->dfas.append()))
				{
					// convert lookahead contents to DFA even if unorderable
					// doubt it matters for any plausible input, but why not
					ch.children.reset();
					ch.children.append({ n_insn, { t_dfa_shortest, idx } });
				}
			}
			if (try_merge_lookaheads(ch))
				ret = true;
		}
		return ret;
	}
	...
	size_t try_convert_sequence_to_nfa(node_t& node, size_t start, size_t end)
	{
		if (start == end)
			return end;
		if (start+1==end && node.children[start].type == n_insn && node.children[start].subtype.type == t_byte)
			return end;
		
		nfa_t nfa;
		nfa.create_nfa(rgx, node.children.slice(start, end-start), node.type==n_alternative);
		
		if (nfa.set_ordering())
		{
			uint32_t idx = rgx->dfas.size();
			bool ok = nfa.convert_to_dfa(rgx->dfas.append());
			if (!ok)
			{
				rgx->dfas.pop_tail();
				goto fallback;
			...
			node.children[start] = { n_insn, { nfa.shortest_first ? t_dfa_shortest : t_dfa_longest, idx } };
			node.children.remove_range(start+1, end);
			return start+1;
		}
		else
		{
	...
	void to_insns_single(node_t& node)
	{
		...
		if (node.type == n_insn)
		{
			if (node.children)
			{
				...
				for (node_t& n : node.children)
					to_insns(n);
				static_assert(t_lookahead_positive+2 == t_lookahead_positive_complete);
				static_assert(t_lookahead_negative+2 == t_lookahead_negative_complete);
				rgx->insns.append({ (insntype_t)(node.subtype.type+2) });
				rgx->insns[start].data = rgx->insns.size();
			}
			else
				rgx->insns.append(node.subtype);
	...
	void to_insns(node_t& node)
	{
		...
		size_t progress_group = SIZE_MAX;
		size_t num_bodies = node.num_total_bodies;
		if ((node.repeat_infinite || node.num_total_bodies > node.num_mandatory_bodies) && node.can_accept_empty)
		{
			// need to ensure optional repeats cannot be empty (that way lies infinite loops, also it's required by the js spec)
			// need to store current pointer at start of loop body, then check if it changed at end of body
			// easiest done by putting it in the captures array
			// I only need as many 'extra' captures as the max nesting, but it's easier to hand out one per loop
			// it's not like potentially-empty loops are common
			// using captures also has the benefit of resetting them to the previous state if backtracked, which is mandatory
			// (I could use an existing capture group, if any, but looking for that sounds like waste of time)
			if (!node.enforce_progress_idx)
				node.enforce_progress_idx = rgx->num_captures_capacity++;
			progress_group = node.enforce_progress_idx;
			if (node.num_total_bodies == node.num_mandatory_bodies)
			{
				// (a?)+ must execute the body at least once, and it can be empty
				// but the repetition cannot be empty, and therefore the body must be compiled to bytecode one more time
				num_bodies++;
				the_infinite_one++;
			}
		}
		...
		for (size_t i=0;i<num_bodies;i++)
		{
			...
			if (progress_group != SIZE_MAX && i >= node.num_mandatory_bodies)
				rgx->insns.append({ t_capture_start, (uint32_t)progress_group });
			to_insns_single(node);
			if (progress_group != SIZE_MAX && i >= node.num_mandatory_bodies)
				rgx->insns.append({ t_ensure_progress, (uint32_t)progress_group });
...
private:
	bool tag_can_accept_empty(node_t& node)
	{
		bool any_child_accepts_empty = false;
		bool any_child_requires_nonempty = false;
		for (node_t& ch : node.children)
		{
			if (tag_can_accept_empty(ch))
				any_child_accepts_empty = true;
			else
				any_child_requires_nonempty = true;
		}
		...
		{
		case n_series:
			can_accept_empty = !any_child_requires_nonempty;
			break;
		case n_alternative:
			can_accept_empty = any_child_accepts_empty;
			break;
		case n_insn:
			if (node.subtype.type == t_byte)
				can_accept_empty = false;
			else if (is_dfa(node.subtype.type))
				can_accept_empty = (rgx->dfas[node.subtype.data].init_state & 0x80000000);
			else
				can_accept_empty = true;
			break;
		default: __builtin_unreachable();
		}
		if (node.num_mandatory_bodies == 0)
			can_accept_empty = true;
		node.can_accept_empty = can_accept_empty;
...
public:
	bool parse(const uint8_t * start, node_t& target)
	{
		...
		flatten_singleton_sequences(target);
		rgx->num_captures_capacity = rgx->num_captures;
		if (at != end)
		{
			rgx->set_fail();
			return false;
...
public:
	...
	static bool compile_unoptimized(regex* rgx, cstring text)
	{
		parser p { rgx, (uint8_t*)text.ptr_raw_end() };
		node_t target;
		if (!p.parse(text.bytes().ptr(), target))
...
private:
	void dump(const node_t& node) const { dump(rgx, node, 0); }
	static void dump(regex* rgx, const node_t& node, int depth)
	{
		printf("%s", &"        "[8-depth]);
		if (node.has_repeat)
		{
...
{
	return parser::compile(this, rgx);
}

bool regex::parse_unoptimized(cstring rgx)
{
	return parser::compile_unoptimized(this, rgx);
}
...
void regex::optimize()
{
	...
	while (again)
	{
		...
		{
			if (insns[i].type == t_jump && is_terminator(insns[insns[i].data].type))
			{
	...
	// doesn't do much, but why not, reduces memory use a little and makes dumps less messy
	array<bitset<256>> new_bytes;
	array<dfa_t> new_dfas;
	for (size_t i=0;i<insns.size();i++)
	{
		...
		if (is_dfa(insns[i].type))
		{
			size_t idx = new_dfas.find(dfas[insns[i].data]);
			if (idx == (size_t)-1)
			{
				idx = new_dfas.size();
				new_dfas.append(dfas[insns[i].data]);
			}
			insns[i].data = idx;
			if (insns[i].type == t_dfa_longest && is_accept(insns[i+1].type))
			{
				if (insns[i+1].type == t_accept)
					insns[i].type = t_dfa_longest_possessive;
				else
					insns[i].type = t_dfa_shortest;
			}
		}
	}
	bytes = std::move(new_bytes);
	dfas = std::move(new_dfas);
...
public:
	...
	class my_stack {
		checkpoint_t* data = data_small;
		size_t elems = 0;
		array<checkpoint_t> data_large;
		checkpoint_t data_small[128];
	public:
		operator bool() const { return elems; }
		...
		void reserve_four()
		{
			if (data == data_small)
			{
				if (elems+4 > ARRAY_SIZE(data_small))
				{
					data_large.reserve(ARRAY_SIZE(data_small)*2);
					data = data_large.ptr();
					memcpy(data, data_small, sizeof(data_small));
				}
			}
			else
			{
				if (elems+4 > data_large.size())
				{
					data_large.reserve(data_large.size()*2);
					data = data_large.ptr();
				}
			}
		}
		forceinline checkpoint_t& pop() { return data[--elems]; }
		forceinline void push(checkpoint_t elem) { data[elems++] = elem; }
	};
	
	const regex * parent;
	const uint8_t * start;
	const uint8_t * end;
	const uint8_t * min_match;
	
	regex::pair* result;
	
	my_stack checkpoints; // keep this last, so member accesses don't require too big offsets
	
	forceinline bool match_inner(checktype_t ty, uint32_t state, const uint8_t * at, const uint8_t * extra)
	{
		static_assert(sizeof(regex::matcher) <= 4096); // so it doesn't require stack probes
#define NEXT(a,b) do { state=a; at=b; goto again; } while(0)
		insntype_t type;
		uint32_t data;
		switch (ty)
		{
	...
	again:
		...
		case t_alternative_first:
			checkpoints.push({ ct_jump, state+1, at, nullptr });
			NEXT(data, at);
		case t_alternative_second:
			checkpoints.push({ ct_jump, data, at, nullptr });
			NEXT(state+1, at);
		...
		case t_dfa_shortest:
		{
			...
			while (true)
			{
				if (dfa_state & 0x80000000)
				{
					...
					if (dfa_state != 0x7FFFFFFF)
						checkpoints.push({ ct_dfa_shortest, state, at, (uint8_t*)(uintptr_t)dfa_state });
					NEXT(state+1, at);
				cont_dfa_shortest:
					data = parent->insns[state].data;
					dfa = &parent->dfas[data];
					dfa_state = (uintptr_t)extra;
				}
		...
		{
			checkpoints.push({ ct_setcapture, state, (uint8_t*)result[data].start, (uint8_t*)result[data].end });
		...
		case t_ensure_progress:
		{
			if ((char*)at == result[data].start)
				return false;
			NEXT(state+1, at);
		}
		...
		case t_lookahead_positive:
			checkpoints.push({ ct_lookahead_pos, data, at, nullptr });
			NEXT(state+1, at);
		case t_lookahead_negative:
			checkpoints.push({ ct_lookahead_neg, data, at, nullptr });
			NEXT(state+1, at);
		case t_lookahead_positive_complete:
			while (true)
			{
				auto pt = checkpoints.pop();
				if (pt.ty == ct_lookahead_pos)
					NEXT(pt.state, pt.data1);
			}
		case t_lookahead_negative_complete:
			while (true)
			{
				auto pt = checkpoints.pop();
...
#undef NEXT
	...
	bool match(const uint8_t * at)
	{
		checkpoints.reset();
		
		checkpoint_t pt = { ct_jump, 0, at, nullptr };
		//size_t m=0;
		while (true)
		{
			if (match_inner(pt.ty, pt.state, pt.data1, pt.data2))
			{
				//m = max(checkpoints.size(), m);
				//if (m > 500)
				//printf("%zu\n", m);
				return true;
			}
...
void regex::match(pair* ret, size_t ret_len, const char * start, const char * at, const char * end, const char * min_match_end) const
{
	array<pair> store;
	pair* result = ret;
	if (UNLIKELY(num_captures_capacity > ret_len))
	{
		store.resize(num_captures_capacity);
		result = store.ptr();
	}
	
	matcher m; // don't just = { this, (uint8_t*)start, (uint8_t*)end, ret }, that makes it zero the checkpoints storage array
	m.parent = this;
	m.start = (uint8_t*)start;
	m.end = (uint8_t*)end;
	m.min_match = (uint8_t*)min_match_end;
	m.result = result;
	
	result[0].start = at;
	if (!m.match((uint8_t*)at))
	{
		if (UNLIKELY(result != ret))
			return;
		// must wipe everything, not just result[0], to avoid bugs where (?!(a)) + "a" leaves trash
		memset(ret, 0, sizeof(pair)*num_captures);
	}
	if (UNLIKELY(result != ret))
		memcpy(ret, result, sizeof(pair)*num_captures);
	else if (num_captures_capacity > num_captures) // must wipe these too
		memset(ret+num_captures, 0, sizeof(pair)*(num_captures_capacity-num_captures));
}

void regex::search(pair* ret, size_t ret_len, const char * start, const char * at, const char * end, const char * min_match_end) const
{
	array<pair> store;
	pair* result = ret;
	if (UNLIKELY(num_captures > ret_len))
	{
		store.resize(num_captures);
		result = store.ptr();
	}
	
	matcher m; // don't tell it { this, (uint8_t*)start, (uint8_t*)end, ret }, that makes it zero the checkpoints array
	m.parent = this;
	m.start = (uint8_t*)start;
	m.end = (uint8_t*)end;
	m.min_match = (uint8_t*)min_match_end;
...
string regex::replace(cstring str, const char * replacement) const
{
	...
	while (true)
	{
		if (m.match((uint8_t*)iter))
		{
			m.min_match = (uint8_t*)iter+1;
...
bool regex_search::parse(cstring rgx)
{
	...
	nfa.create_nfa(&r, node.children, node.type==regex::parser::n_alternative);
	regex::dfa_t dfa;
	if (!nfa.convert_to_dfa(dfa))
		return false;
