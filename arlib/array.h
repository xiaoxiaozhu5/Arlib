...
public:
	...
	{
		const T * found = (T*)memmem(items, count*sizeof(T), sequence.ptr(), sequence.size()*sizeof(T));
		if (found) return found - this->items;
		else return -1;
	}
	bool contains(arrayview<T> sequence) const requires (std::is_same_v<T, uint8_t>)
	{
		return memmem(items, count*sizeof(T), sequence.ptr(), sequence.size()*sizeof(T));
	}
...
#define BITARRAY_ASM
	...
	union {
		chunk_t bits_inline[n_inline/chunk_size];
		chunk_t* bits_outline;
	};
	size_t nbits;
